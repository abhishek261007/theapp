...
function averageHexColors(colors: string[]): string | undefined {
  ...
  r = Math.floor(Math.round(r / colors.length) * 0.775);
  g = Math.floor(Math.round(g / colors.length) * 0.775);
  b = Math.floor(Math.round(b / colors.length) * 0.775);
...
export default function DesignDetailsScreen() {
  ...
  const [catalogColor, setCatalogColor] = useState<string | undefined>();
...
const wishlistItems  = useWishlistStore((s) => s.items);
  ...
  useEffect(() => {
    async function fetchDesigns() {
      try {
        ...
        const hexes = available.map((d) => d.dominantColor).filter(Boolean) as string[];
        if (hexes.length > 0) {
          setCatalogColor(averageHexColors(hexes));
        }
    ...
    if (params.catalogId) {
      ...
      setCatalogColor(params.dominantColor ?? undefined);
      setLoading(false);
    }
  ...
  const renderItem = useCallback(
    ...
    [openModal, designs.length, insets.bottom, hasWishlist, toggleWishlist, imageRefs, catalogColor]
  );
  ...
  if (loading) {
    return (
      <View style={{ flex: 1, backgroundColor: C.NAVY_DEEP, justifyContent: 'center', alignItems: 'center' }}>
        ...
        <Text style={{ fontFamily: 'Helvetica', fontWeight: '600', fontSize: 28, color: C.GOLD, opacity: 0.5, letterSpacing: 2 }}>
          ◆
        </Text>
        <Text style={{ fontFamily: 'Helvetica', fontWeight: '600', fontSize: 10, letterSpacing: 2, textTransform: 'uppercase', color: 'rgba(255,255,255,0.6)', marginTop: 14 }}>
          Loading
        </Text>
      </View>
    );
      ...
      <View style={styles.safe}>
        ...
        >
          <Animated.View style={[styles.modalContainer, { opacity: modalOpacity }]}>
            ...
            {modalImageUrl && (
              // @ts-ignore - react-native-image-pan-zoom types are outdated for React 18 children
              <ImageZoom
                cropWidth={SCREEN_W}
                cropHeight={SCREEN_H}
                imageWidth={SCREEN_W}
        ...
        <View style={styles.body}>
          ...
          onScroll={(e) => {
            rawIndexRef.current = Math.round(e.nativeEvent.contentOffset.x / SCREEN_W);
          }}
...
function DesignPage({ item, index, total, bottomInset, onOpenModal, isWishlisted, onToggleWishlist, setImageRef, catalogColor }: DesignPageProps) {
  const C = useColors();
  const pageS = useMemo(() => createPageStyles(C), [C]);
    ...
    <View style={pageS.page}>
      <View style={pageS.container}>
        ...
        <ImageGallery
          ...
          item={{ ...item, dominantColor: catalogColor || item.dominantColor }}
...
function createPageStyles(c: Colors) {
...
function FooterActionTray({ item, isInCart, isSold, bottomInset, imageRefs, rawIndexRef, onAddToCart }: any) {
  ...
  }, [item, isSold, triggerFlyToCart, onAddToCart, imageRefs, rawIndexRef]);
